(function() {
    // =========================================================================================================
    // ==== Большую часть кода этой игры я взял из web-источника, поскольку в настоящий момент              ====
    // ==== (из-за уровня моих знаний и опыта) я бы очень долго писал ее с нуля. Я пишу об этом здесь,      ====
    // ==== что бы в дальнейшем не сложилось завышенное впечатление моих знаний. Но я попытался реализовать ====
    // ==== все пункты из ТЗ, добавить весь нужный функционал. Готовое решение переписывал вручную, что бы  ====
    // ==== понимать работу кода и в процессе некого "интенсива" получить знания, как минимум это было      ====
    // ==== информативно и интересно :)                                                                     ====
    // =========================================================================================================
	
    let startGame = false; // Запускает игру по нажатию кнопки "играть" и блокирует возможность изменения расстановки кораблей
    let isHandlerPlacement = false; // Обработчик события ручного размещения кораблей и изменение их положения
    let isHandlerController = false; // Обработчик собитый во время игры
    let compShot = false; // Блокировка действий игрока во время хода компьютера 


    const getElement = id => document.getElementById(id); // Получим объект DOM по id

    // Получим координаты элемента относительно viewport'а
    ...
    // Получим поле компьютера
    const computerfield = getElement('field_computer');

    // ========= Класс игрового поля =========
    class Field {

        static FIELD_SIDE = 330; // Размер игрового пол в px
        static SHIP_SIDE = 33; // Размеры корабля в px

        // Объект с данными кораблей. Ключом является тип корабля, а значением массив. 
        // Первый элемент значения указывает на кол-во кораблей такого типа, а второй элемент указывает на кол-во палуб у корабля.
        static SHIP_DATA = {
            fourdeck: [1, 4],
            tripledeck: [2, 3],
            doubledeck: [3, 2],
        ...
        constructor (field) {
            this.field = field; // Объект на игровом поле, который получен в качестве аргумента
            this.squadron = {}; // Пустой объект, куда будут занесены данные по созданным кораблям 
            this.matrix = []; // В этот массив заносятся координаты кораблей, попаданий и пустых клеток
            
            // Получаем координаты всех 4-ех сторон игрового поля относительно начала document
        ...
        // Данная функция формирует массив и заполняет его нулями, в который будут записываться координаты кораблей и попаданий, промахов, а так же отметки пустых клеток
        static createMatrix() {
            return [...Array(10)].map(() => Array(10).fill(0));
        }

        // n - максимальное значение, которое хотим получать
        ...
        // Очищает игровое поле, объект "squadron", массив "matrix"
        cleanField() {
            // Удаляет все объекты с игрового поля
            while (this.field.firstChild) {
                this.field.removeChild(this.field.firstChild);
            }

            // Удаляет все элементы объекта кораблей
            this.squadron = {};
            // Очищает матрицу игрового поля
            this.matrix = Field.createMatrix();

        }

        // Запускает перебор объекта "SHIP_DATA" с данными кораблей по каждому типу и создаёт экземпляр каждого корабля с заданными свойствами
        randomLocationShips() {
            for (let type in Field.SHIP_DATA) {
                // Кол-во кораблей данного типа
                let count = Field.SHIP_DATA[type][0];
                // Кол-во палуб корабля данного типа
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    // Кол-во палуб
                    options.decks = decks;
                    // Имя корабля (нужно для его идентификации)
                    options.shipname = type + String(i + 1);
                    // Экземпляр коробля со свойставами, указанными в объекте options
        ...
        // Проверка координат коробля 
        checkLocationShip(obj, decks) {
            let {x, y, kx, ky, fromX, toX, fromY, toY} = obj;

            // Ограничивает по оси Х. Если координаата х равна нулю, то палуба расположена в самой верхней строке (примыкает к верхней границе) и началом цикла будет строка с индексом 0, в противном случае, нужно начать проверку со строки с индексом на единицу меньшим, чем у исходной, т.е. находящейся выше исходной строки
            fromX = (x == 0) ? x : x - 1;

            // Если условие истинно - значит корабль расположен вертикально и его последняя палуба примыкает к нижней границе поля
            if (x + kx * decks == 10 && kx == 1) toX = x + kx * decks;

            // Корабль расположен вертикально и между ним и нижней границей поля есть хотя бы одна строка, то координата этой строки будет индексом конца цикла
            ...
            // формируем индексы начала и конца выборки по столбцам принцип такой же, как и для строк
            fromY = (y == 0) ? y : y - 1;
            ...
            // отфильтровываем ячейки, получившегося двумерного массива, содержащие 1, если такие ячейки существуют - возвращаем false
    ...
    class Ships {
        ...
        constructor (self, {x, y, kx, ky, decks, shipname}) {
            this.player = (self === human) ? human : computer; // С каким экземпляром работаем
            this.field = self.field // На какком поле создается корабль
            this.shipname = shipname; // Уникальное имя корабля
            this.decks = decks; // Кол-во палуб
            this.x = x; // Координаты Х первой палубы
            this.y = y; // Координаты Y первой палубы
            this.kx = kx; // Направление расположения палуб
            this.ky = ky; // Направление расположения палуб
            this.hits = 0; // Счётчик нападений
            this.arrDecks = []; // Массив с координатами палуб корабля
        }
        ...
        static showShip(self, shipname, x, y, kx) {
            ...
            // через атрибут 'style' задаём позиционирование кораблю относительно его родительского элемента.
            // Cмещение вычисляется путём умножения координаты первой палубы на размер клетки игрового поля, этот размер совпадает с размером палубы
        ...
        createShip() {
            ...
            while (k < decks) {
                ...
                // значение 1, записанное в массив, говорит что по данным координатам находится палуба некого корабля
                player.matrix[i][j] = 1;
            ...
            if (player === human) {
                ...
                // когда количество кораблей в эскадре достигнет 10, т.е. все корабли сгенерированны, то можно показать кнопку запуска игры
    ...
    // ========= Класс размещения кораблей =========
    class Placement {
        // Объект с координатами игрового поля
        static FRAME_COORDS = getCoordinates(humanfield);

        constructor() {
            this.dragObject = {}; // Объект перетаскиваемого корабля
            this.pressed = false; // Флаг нажатия на левую кнопку мыши
        };

        // Имя перетаскиваемого корабля
        static getShipName = el => el.getAttribute('id');

        // Кол-во палуб у перетаскиваемого корабля
        static getCloneDecks = el => {
            // Убераем из имени корабля последний символ 
            const type = Placement.getShipName(el).slice(0, -1);
            return Field.SHIP_DATA[type][1];
        };

        setObserver() {
            // если обработчики уже установлены, выходим из функции
            ...
            // флаг установки обработчиков
            isHandlerPlacement = true;
        };

        // Проверет, что нажатие было на левую кнопку мыши. Определяет корабль, который перемащают. Если нажатие было в пустой области, то работа функции прекращается.
        // dragObject - записываются свойства переносимого элемента, если перенос законился вне игрового поля, то возвращает корабль в исходное место
        onMouseDown(e) {
        ...
        creatClone() {
            ...
            clone.rollback = () => {
                // Редактирование положения корабля
    ...
    class Controller {
		...
		init() {
			...
			if (this.player === human) {
				compShot = false;
				this.text = `${namePlayer}, вы стреляете первым`;
			} else {
				compShot = true;
				this.text = `${namePlayer}, первым стреляет компьютер`;
				// выстрел компьютера
		...
		makeShot(e) {
			...
			switch(v) {
				...
				case 4:
					Controller.showServiceText(`${namePlayer}, по этим координатам вы уже стреляли!`);
					break;
			}
		}
		...
		miss(x, y) {
			...
			if (this.player === human) {
				text = `${namePlayer}, вы промахнулись. Стреляет компьютер.`;
				this.player = computer;
				this.opponent = human;
				compShot = true;
				setTimeout(() => this.makeShot(), 2000);
			} else {
				text = `${namePlayer}, компьютер промахнулся. Ваш выстрел.`;
		...
		hit(x, y) {
			...
			text = (this.player === human) ? `${namePlayer}, поздравляем! Вы попали. Ваш выстрел.` : `${namePlayer}, компьютер попал в ваш корабль. Выстрел компьютера`;
			setTimeout(() => Controller.showServiceText(text), 400);
			...
			if (Object.keys(this.opponent.squadron).length == 0) {
				if (this.opponent === human) {
					text = `${namePlayer}, к сожалению, вы проиграли.`;
					...
					text = `${namePlayer}, поздравляем! Вы выиграли!`;
				}
    ...
    const instruction = getElement('instruction'); // Получим объект в DOM с инструкцией
    const shipsCollection = getElement('ships_collection'); // Получим объект DOM, в котором будут размещаться корабли для перетаскивания
    const initialShips = document.querySelector('.wrap + .initial-ships'); // Объект DOM с набором кораблей, которые можно перетаскивать
    const toptext = getElement('text_top'); // Заголовок
    const buttonPlay = getElement('play'); // Кнопка играть
    const buttonNewGame = getElement('newgame'); // Кнопка перезапуска игры

    // const namePlayer = prompt('Привет! Прежде чем начать играть, давай познакомимся. Как твоё имя?') // Имя игрока
    // // Валидация имени
    // if (namePlayer == '' || namePlayer == ' ') {
    //     alert('Введите имя!');
    //     namePlayer = prompt('Привет! Прежде чем начать играть, давай познакомимся. Как твоё имя?')
    // }
    // toptext.innerHTML = `${namePlayer}, расставьте корабли`; // Выводим имя в заголовке

    // Имя игрока
	var namePlayer = document.getElementById('inp1')
	const btnName = document.getElementById('btn-name')
	const divNamePlayer = document.getElementById('name-player');
	...
	btnName.addEventListener('click', () => {
        ...
        if (!namePlayer) {
			errMess.innerHTML = "Введите имя/никнейм"
			setTimeout(() => window.location.reload(), 2000)
		} else if (namePlayer.length <= 1) {
            errMess.innerHTML = "Имя/никнейм должно быть больше 1 символа"
			setTimeout(() => window.location.reload(), 2000)
        } else {
			setTimeout(() => {divNamePlayer.style.display = 'none'}, 200)
		}

		toptext.innerHTML = `${namePlayer}, расставьте корабли`;

	});
    
    
    const human = new Field(humanfield); // Получаем экземпляр поля игрока
    
    let computer = {}; // Регестрируем игровое поле компьютера

    let control = null;


    getElement('type_placement').addEventListener('click', e => {
        // Используем target (делегирование событий) что бы не вашать 2 обработчика на кнопки "Играть" и "Перезапустить"
        if(e.target.tagName != 'SPAN') return;

        // Скрывает кнопку "играть" во время расстановки кораблей
        buttonPlay.hidden = true;

        // Очищает поле игрока, если пытались расставить корабли
        human.cleanField();

        // Очищает клон объекта с набором кораблей
        let initialShipsClone = '';

        // Способ расстановки кораблей на игровом поле
        const type = e.target.dataset.target;

        // Объект typeGeneration со свойствами, которым соответствуют функции, в которых вызывается нужная расстановка кораблей(случайная/ручная)
        const typeGeneration = {
            random() {
                // Скрывает контейнер с кораблями, которые можно перетягивать для расстановки
                shipsCollection.hidden = true;
                // Вызов функции для случайной расстановки
                human.randomLocationShips();

            },

            manually() {
                // Определяет видимость набора кораблей
                let value = !shipsCollection.hidden;

                // Если в контейнере кроме инфо строки есть еще набор кораблей, то удаляет их
                if (shipsCollection.children.length > 1) {
                    shipsCollection.removeChild(shipsCollection.lastChild);
                }

                // Если набор кораблей при клике был невидимым, то клонируем его, переносим в игровой контейнер и показываем
                if (!value) {
                ...
                // В зависимости от значения value показываем или скрываем набор кораблей
                shipsCollection.hidden = value; 
            }
        };

        // Вызов функции объекта в зависимости от способа расстанвоки кораблей
        typeGeneration[type]();

        // Создаем экземпляр класса, который отвечает за перетаскивание и редактирование положения кораблей
    ...
    buttonPlay.addEventListener('click', e => {
        ...
        toptext.innerHTML = `Бой ведут: ${namePlayer} и компьютер`;
